...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (useBlocks) {
      ...
      const galleryBlockIds = (blocks as DropBlock[] | null)?.filter((b) => b.type === "image_gallery").map((b) => b.id) || [];
      const galleryImagesMap: Record<string, GalleryImage[]> = {};
      ...
      if (galleryBlockIds.length > 0) {
        ...
        // Group by block_id
        (galleryImages as GalleryImage[] | null)?.forEach((img) => {
      ...
      const enrichedBlocks: DropBlock[] = (blocks as DropBlock[] | null)?.map((block) => ({
        ...block,
      ...
      const blocks: DropBlock[] = (dropPosts as DropPost[] | null)?.map((dp, index) => ({
        id: `post-${index}`,
        drop_id: dropId,
        type: "post" as const,
