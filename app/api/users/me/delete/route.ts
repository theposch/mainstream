...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Delete avatar from Supabase Storage if it was uploaded there (best-effort)
    const avatarUrl = userProfile?.avatar_url ?? '';
    const AVATAR_MARKER = '/storage/v1/object/public/avatars/';
    const markerIdx = avatarUrl.indexOf(AVATAR_MARKER);
    if (markerIdx !== -1) {
      const storagePath = avatarUrl.slice(markerIdx + AVATAR_MARKER.length);
      if (storagePath) {
        await adminSupabase.storage.from('avatars').remove([storagePath]).catch((e: unknown) => {
          console.error('[DELETE /api/users/me/delete] Failed to delete avatar from storage:', e);
        });
      }
    }
